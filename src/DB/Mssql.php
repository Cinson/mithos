...
class Mssql {
   ...
   public function query($sql = null, $params = array()) {
       if (!Connection::isConnected()) {
           throw new Exception\ConnectionException('Can not find a Connection to MSSQL');
       }

       if (!$result = @mssql_query($this->prepare($sql, $params), Connection::getConnection())) {
           throw new Exception\QueryException(mssql_get_last_message() . ' SQL: ' . $this->prepare($sql, $params));
       }
   ...
   public function value($data, $type = null) {
       ...
       if (in_array($type, ['integer', 'float', 'binary']) && is_numeric($data)) {
           return $data;
       }
       return "'" . $data . "'";
   }

   public function prepare($sql = null, $params = []) {
       if (!empty($params)) {
